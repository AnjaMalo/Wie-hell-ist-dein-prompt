...
export default async (req) => {
  ...
  // Schlüssel robust einlesen: Es kommt vor, dass im Feld mehrere Werte oder Zeilenumbrüche stehen.
  // Wir suchen deshalb alle gültig aussehenden Schlüssel heraus und nehmen den zuletzt eingetragenen.
  const raw = Netlify.env.get("ANTHROPIC_API_KEY") || Netlify.env.get("ANTHROPIC_KEY") || "";
  if (!raw) { console.error("check: ANTHROPIC_API_KEY fehlt"); return json({ error: "config" }, 500); }
  const found = String(raw).match(/sk-ant-[A-Za-z0-9_\-]{20,}/g) || [];
  const key = found.length ? found[found.length - 1] : String(raw).trim();
  // Harmlose Kennzahlen für die Fehlersuche, der Schlüssel selbst wird nie protokolliert
  const keyInfo = `Rohwert ${String(raw).length} Zeichen, ${found.length} Schlüssel erkannt, verwendet ${key.length} Zeichen, Präfix ${key.startsWith("sk-ant-") ? "ok" : "unerwartet"}`;
